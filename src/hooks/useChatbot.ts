...
const generateResponse = (question: string): string => {
  ...
  // Brze teme: uvijek otvori točno zadanu temu (bez "krive" detekcije)
  if (question.startsWith(QUICK_TOPIC_PREFIX)) {
    const topic = question.slice(QUICK_TOPIC_PREFIX.length);
    switch (topic) {
      case "oprema":
        return renderOprema(izreka);
      case "kaciga":
        return renderKaciga(izreka);
      case "prometni_znakovi":
        return renderPrometniZnakovi(izreka);
      case "pravila":
        return renderPravila(izreka);
      case "nocna_voznja":
        return renderNocnaVoznja(izreka);
      case "raskrizje":
        return renderRaskrizje(izreka);
      default:
        break;
    }
  }
  ...
  if (suggestions.length > 0) {
    return `🤔 Hmm, nisam siguran što točno pitaš, ali možda te zanima nešto od ovoga:\n\n${suggestions.map((s, i) => `${i + 1}. "${s}"`).join("\n")}\n\nKlikni na jednu od brzih tema ili mi postavi pitanje na drugi način.\n\n🗣️ ${izreka}`;
  }

  // Default odgovor
  return `🤔 Nisam te baš razumio! Hvala što pitaš — probaj malo drugačije.\n\nMogu ti pomoći s temama kao što su:\n• 🚲 Oprema bicikla\n• 🚸 Prometni znakovi\n• 📋 Pravila vožnje\n• 🔀 Ponašanje na raskrižju\n• 🌙 Noćna vožnja\n• 📖 Definicije pojmova iz prometa\n\nKlikni na neku od brzih tema gore ili pritisni ✨ za nasumično pitanje!\n\n🗣️ ${izreka}`;
};
